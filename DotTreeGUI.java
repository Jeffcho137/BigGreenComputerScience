...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			Dot p = new Dot (x, y);
			if(tree!=null)tree.insert(p); //if the tree's there, insert the node
			else tree = new PointQuadtree(p,0,0,width,height);
		}
		else if (mode == 'q') {
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(350, 430), 0, 0, 800, 600); //A
		tree.insert(new Dot(330,190)); // B
		tree.insert(new Dot(560,230)); // C
		tree.insert(new Dot(470,230)); // D
		tree.insert(new Dot(690,530)); // E
		int bad = 0;
		bad += testFind(330,190,10,5,3,1); 	
		bad += testFind(690,530,10,3,2,1);
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		g.fillOval((int)tree.getPoint().getX()-mouseRadius/2, (int)tree.getPoint().getY()-mouseRadius/2, mouseRadius,mouseRadius);	
		g.drawLine((int)tree.getPoint().getX(), 0, (int)tree.getPoint().getX(), height);
		g.drawLine(0, (int)tree.getPoint().getY(), width, (int)tree.getPoint().getY());
		...
		for (int i = 1; i <= 4; i++) {
			if (tree.getChild(i) != null) {
				drawTree(g, tree.getChild(i),level+1, tree);
			}
		}
	}
	
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level, PointQuadtree<Dot> parent) {
		// Set the color for this level
		g.setColor(rainbow[level % rainbow.length]);
		// Draw this node's dot and lines through it
		// TODO: YOUR CODE HERE
		g.fillOval((int)tree.getPoint().getX()-mouseRadius/2, (int)tree.getPoint().getY()-mouseRadius/2, mouseRadius,mouseRadius);	
		// quadrant 1 or 4
		if((int)tree.getPoint().getX()>(int)parent.getPoint().getX()) {
			g.drawLine((int)parent.getPoint().getX(), (int)tree.getPoint().getY(), (int)parent.getX2(), (int)tree.getPoint().getY());
		}
		// quadrant 2 or 3
		else g.drawLine((int)parent.getX1(), (int)tree.getPoint().getY(), (int)parent.getPoint().getX(), (int)tree.getPoint().getY());
		
		// quadrant 1 or 2
		if((int)tree.getPoint().getY()<(int)parent.getPoint().getY()) {
			g.drawLine((int)tree.getPoint().getX(), (int)parent.getY1(), (int)tree.getPoint().getX(), (int)parent.getPoint().getY());
		}
		// quadrant 3 or 4
		else g.drawLine((int)tree.getPoint().getX(), (int)parent.getPoint().getY(), (int)tree.getPoint().getX(), (int)parent.getY2());
